...
try:
    ...
    print("Knk 10 saniye bekle modüller iniyo ...")
...
def get_number():
  ...
  KARŞI TARAFIN TELEFON NUMARASI NE???..."53xxxxxxxx" \n[?] : """))
            if len(str(number)) == 10 and str(number)[0] == "5":
                return number
            else:
                clear()
                print(f"YAZDIĞIN NUMARA YANLIŞ KK.")
        except:
            clear()
            print(f"Lütfen Bir Numara Yaz .")

def get_amount():
    global clear
    while True:
        try:
            amount = int(input(f"""Kaç Sms Gönderilsin Sınırsız Gönderim İçin "0" Yaz Bas.\n[?] : """))
            if amount >= 0:
                return amount
            else:
                clear()
                print(f"Girdigin Sayı 0 Dan Küçük Aptal oc.")
        except:
            clear()
            print(f"Sayı Gir Birader.")

def get_worker_amount():
    global clear
    while True:
        try:
            worker_amount = int(input(f"Thread Sayısıni Gir. Tavsiye Edilen 5-100 arasıdır.\n[?] : "))
            if worker_amount >= 1:
                return worker_amount
            else:
                clear()
                print(f"Girilen Sayı 1 Den Küçük Oç.")
        except:
            clear()
            print(f"Sayı Gir Kanka.")

def restart():
    global clear
    while True:
        question = input(f"Programdan Çıkılsınmı Oç.\n[Y/N] : ").upper().replace(" ", "")
        if question == "Y":
            quit()
        elif question == "N":
            clear()
            ...
            clear()
            print(f"Yanlış Tuşa Bastın Oc malı.")
